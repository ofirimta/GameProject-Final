...
void Screen::setInventoryAt(const Player& my_player, int room)
{
	...
	if (player_sign == '$')
	{
		while (ch != '\0')
		{
			...
			gotoxy(52 + i, 1);                     // עדכון מסך: לך למיקום
			cout << ch;
			i++;
...
void Screen::delInventoryAt(const Player& my_player, int room)
{
	...
	if (player_sign == '$')
	{
		...
		while ((ch != ' ') && (ch != '&'))
		{
			...
			gotoxy(52 + i, 1); // תיקון: מחיקה ויזואלית מהמסך
			cout << ' ';
			i++;
	...
	else
	{
		...
		while ((ch != ' ') && (ch != '\0'))
		{
			...
			gotoxy(65 + i, 1); // תיקון: מחיקה ויזואלית מהמסך
			cout << ' ';
			i++;
...
void Screen::setMessageAt(int room, const char* new_str)
{
	...
	while (ch != '\0')
	{
		...
		gotoxy(10 + i, 0); // תיקון: הדפסה למסך (שורה 0)
		cout << ch;
		i++;
		ch = new_str[i];
	}
...
void Screen::setHintAt(int room, const char* new_str)
{
	...
	while (ch != '\0')
	{
		...
		gotoxy(7 + i, 2); // תיקון: הדפסה למסך (שורה 2)
		cout << ch;
		i++;
		ch = new_str[i];
	}
...
void Screen::delMessageAt(int room)
{
...

	{
		...
		while ((ch != ' ') && (ch != '\0'))
		{
			...
			gotoxy(10 + i, 0); // תיקון: מחיקה מהמסך
			cout << ' ';
			i++;
